...
describe("buildTimeSlots", () => {
  it("returns (365 + 30) * 20 timeslots", () => {
    expect(buildTimeSlots().length).toEqual(
      (365 + 30) * 20
    );
  });
...
describe("generateFakeAppointments", () => {
  ...
  it("generates more than half of all the timeslots available", () => {
    const result = generateFakeAppointments(
      customers,
      timeSlots
    );
  ...
  it("sets the stylist", () => {
    ...
    expect(
      ["Ashley", "Jo"].includes(result[0].stylist)
    ).toBeTruthy();
  });
  ...
  it("picks a random service", () => {
    ...
    expect(
      services.includes(result[0].service)
    ).toBeTruthy();
  });
});
...
describe("appointments", () => {
  const timeSlots = [
    { startsAt: 123, stylists: ["Jo"] },
    { startsAt: 234, stylists: ["Jo"] },
  ];
  ...
  it("initially loads all the initialAppointments", () => {
    ...
    const result = appointments.getAppointments(
      0,
      124,
      []
    );
  ...
  describe("add", () => {
    it("removes the time from available time slots", () => {
      ...
      appointments.add({
        ...
        stylist: result[0].stylists[0],
      });
      expect(
        appointments.getTimeSlots().length
      ).toEqual(1);
    });
  });
  ...
  describe("errors", () => {
    ...
    it("returns no errors for a unique object", () => {
      ...
      expect(
        appointments.errors(appointment)
      ).toEqual({});
    });

    it("returns error if the timeslot is already used for that stylist", () => {
      const appointment = {
        startsAt: 1,
        stylist: "Jo",
        customer: "customer",
    ...
    it("does not return error if the timeslot is used but for a different stylist", () => {
      const appointment = {
        startsAt: 1,
        stylist: "Sam",
        customer: "customer",
  ...
  describe("isValid", () => {
    ...
    it("returns true for a unique object", () => {
      ...
      expect(
        appointments.isValid(appointment)
      ).toBeTruthy();
    });

...
describe("app", () => {
  ...
  function spyOn(object, method, spy) {
    originalFunctions[object] =
      originalFunctions[object] || {};
    originalFunctions[object][method] =
      object[method];
    object[method] = spy;
  }
  ...
  describe("GET appointments", () => {
    ...
    it("returns the result of calling getAppointments", async () => {
      ...
      appointmentsSpy.mockReturnValue(
        allAppointments
      );
      ...
      it("selects the customer with the given id", async () => {
        await request(app())
          ...
          .then((response) => {
            ...
            expect(data.customer.firstName).toEqual(
              "Daniel"
            );
          });
      });
      ...
      it("appends the appointment to the customer", async () => {
        ...
        await request(app())
          ...
          .then((response) => {
            ...
            ).toEqual([{ startsAt: "123" }]);
          });
      });
    });

    describe("availableTimeSlots query", () => {
      let getTimeSlotsSpy = jest.fn(() => [
        { startsAt: 123 },
      ]);
      ...
      it("calls appointments.getAppointments", async () => {
        await request(app())
          ...
          .then(() => {
            expect(
              appointmentsSlotsSpy
            ).toBeCalledWith(123, 234, allCustomers);
          });
      });

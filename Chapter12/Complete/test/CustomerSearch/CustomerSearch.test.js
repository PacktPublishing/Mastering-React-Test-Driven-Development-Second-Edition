...
jest.mock("../../src/CustomerSearch/SearchButtons", () => ({
  SearchButtons: jest.fn(() => <div id="SearchButtons" />),
}));
...
describe("CustomerSearch", () => {
  ...
  it("displays provided action buttons for each customer", async () => {
    const actionSpy = jest.fn().mockReturnValue("actions");
    global.fetch.mockResolvedValue(
      fetchResponseOk(oneCustomer)
    );
    await renderAndWait(
  ...
  it("passes customer to the renderCustomerActions prop", async () => {
    ...
    await renderAndWait(<CustomerSearch {...testProps} />);
    expect(testProps.renderCustomerActions).toBeCalledWith(
      oneCustomer[0]
    );
  });

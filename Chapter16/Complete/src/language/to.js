...
const insertParameterValues = (parameters, state) =>
  mapObjectValues(parameters, (value) => {
    if (isParameterReference(value))
      return parameterValue(value.substring(1)).get(
        state
      );
    return value;
  });
...
const performCall = (
  ...
  const instructionsWithParameterValues =
    innerInstructions.map((instruction) => ({
      ...instruction,
  ...
  return performAll(
    state,
    instructionsWithParameterValues
  );
};
...
const performTo = (state, instruction) => {
  ...
  if (
    existingFunction &&
    existingFunction.isWriteProtected
  ) {
    throw {
